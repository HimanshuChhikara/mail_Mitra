      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            {!isLoggedIn && !isGeneratePage && !isDashboardPage && (
              <button
                onClick={handleSignIn}
                disabled={isConnecting}
                className="hidden sm:flex items-center gap-2 text-gray-300 hover:text-white text-sm font-medium transition-colors px-3 py-2 rounded-lg hover:bg-slate-800"
              >
                {isConnecting ? (
